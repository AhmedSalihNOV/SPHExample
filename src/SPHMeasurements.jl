...
export MeasurementPoint, MeasurementLine, MeasurementGrid,
       ...
       SimulationMeasurement, SimulationMeasurements, perform_measurements!,
...
"""
    SimulationMeasurement(region, field)
...
"""
struct SimulationMeasurement{R<:MeasurementRegion}
    region::R
    field::Symbol
    data::Dict{Float64,Any}
    SimulationMeasurement(region::R, field::Symbol) where {R<:MeasurementRegion} =
        new{R}(region, field, Dict{Float64,Any}())
end

"""
    SimulationMeasurements(measurements, times)

Holds a collection of `measurements` to be sampled at the simulation
`times` provided.
"""
struct SimulationMeasurements
    measurements::Vector{SimulationMeasurement}
...
`data` dictionary.
"""
function perform_measurements!(sm::SimulationMeasurements, time, particles, constants,
                               kernel::SPHKernelInstance)
...
"""
function save_measurement_vtk(measurement::SimulationMeasurement, time, filepath)
